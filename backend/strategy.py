...
from .sidecar_detector import get_sell_sidecar_dates, get_buy_sidecar_dates
...
def is_sell_sidecar_date(date_value: str) -> bool:
    return str(date_value) in get_sell_sidecar_dates()


def is_buy_sidecar_date(date_value: str) -> bool:
    return str(date_value) in get_buy_sidecar_dates()
...
def latest_status(df: pd.DataFrame) -> dict:
    ...
    sell_sidecar_today = is_sell_sidecar_date(row["date"])
    buy_sidecar_today = is_buy_sidecar_date(row["date"])
    ...
    return {
        ...
        "sell_sidecar_today": sell_sidecar_today,
        "buy_sidecar_today": buy_sidecar_today,
...
def backtest_strategy(df: pd.DataFrame) -> list:
    ...
    for i in range(65, len(df)):
        ...
        trade = {
            ...
            "entry_type": "AUTO_SELL_SIDECAR_REVERSE_BUY",
            "ma20_up": bool(row["ma20_slope"] > 0),

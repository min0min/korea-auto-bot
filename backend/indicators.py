...
def add_indicators(df: pd.DataFrame, ma_slope_days: int = 5, rsi_period: int = 14) -> pd.DataFrame:
    df = df.copy()
    df["ma5"] = df["close"].rolling(5).mean(); df["ma20"] = df["close"].rolling(20).mean(); df["ma60"] = df["close"].rolling(60).mean(); df["ma120"] = df["close"].rolling(120).mean()
    ...
    df["drop_5d"] = df["close"] / df["close"].shift(5) - 1
